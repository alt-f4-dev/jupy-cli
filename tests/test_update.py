...
class SemVerTests(unittest.TestCase):
    def test_basic_ordering(self):
        self.assertLess(
            update.SemVer.parse("0.0.1"),
            update.SemVer.parse("0.0.2"),
        )
        self.assertLess(
            update.SemVer.parse("1.9.9"),
            update.SemVer.parse("2.0.0"),
        )
        self.assertEqual(
            update.SemVer.parse("v1.2.3"),
            update.SemVer.parse("1.2.3"),
        )

    def test_prerelease_ordering(self):
        self.assertLess(
            update.SemVer.parse("1.0.0-alpha"),
            update.SemVer.parse("1.0.0"),
        )
        self.assertLess(
            update.SemVer.parse("1.0.0-alpha.1"),
            update.SemVer.parse("1.0.0-alpha.beta"),
        )
...
class LayoutTests(unittest.TestCase):
    ...
    def test_locate_release_root(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            source = self.make_release_tree(
                extracted,
                "posix",
            )

            self.assertEqual(
                update.locate_release_root(
                    extracted,
                    "posix",
                ),
                source.resolve(),
    ...
    def test_validate_release_tree(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            update.validate_release_tree(
                source,
                release,
                "posix",
            )

    def test_validate_windows_release_tree(self):
        with tempfile.TemporaryDirectory() as temporary:
            source = self.make_release_tree(
                Path(temporary),
                "nt",
    ...
    def test_posix_manifest_preserves_external_symlinks(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            manifest = update.install_manifest(
                source,
                root,
                bin_dir,
                "posix",
    ...
    def test_apply_update_to_regular_launchers(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (root / "core" / "jupy_core.py").write_text(
                "OLD = 1\n",
                encoding="utf-8",
            )
            (root / "core" / "jupy_update.py").write_text(
                "OLD = 1\n",
                encoding="utf-8",
            )
            ...
            self.assertEqual(
                (bin_dir / "jupyup").read_text(encoding="utf-8"),
                "launcher\n",
            )

    def test_apply_update_windows_launchers(self):
        with tempfile.TemporaryDirectory() as temporary:
            base = Path(temporary)
            source = self.make_release_tree(
                base / "source",
            ...
            self.assertEqual(
                (bin_dir / "jupyup.cmd").read_text(encoding="utf-8"),
                "launcher\n",
            )
            self.assertEqual(
                (bin_dir / "jupyup.ps1").read_text(encoding="utf-8"),
                "launcher\n",
            )
